...
public record CreationResponse (int id, String title, String publisher, int age_limit, int episodes, boolean is_end, int free_episodes, int gidamu, String period, int rent_cost, int buy_cost, Date start_date, Date latest_date, String description, List<Creator> creatorList, List<Platform> platformList, List<Genre> genreList, List<Keyword> keywordList){
    public static CreationResponse from(Creation creation){
        return new CreationResponse(creation.getCreation_id(), creation.getTitle(), creation.getPublisher(), creation.getAge_limit(), creation.getEpisodes(), creation.is_end(), creation.getFree_episodes(), creation.getGidamu(), creation.getPeriod(), creation.getRent_cost(), creation.getBuy_cost(), creation.getStart_date(),creation.getLatest_date(), creation.getDescription(), creation.getCreatorList(),creation.getPlatformList(),creation.getGenreList(),creation.getKeywordList());
    }
}
